...
mod test {
    ...
    #[cfg(feature = "internal-api")]
    #[test]
    fn test_ensure_read_supported_with_custom_override() {
        use crate::actions::{Metadata, Protocol};
        use crate::schema::{DataType, StructField, StructType};
        use crate::table_features::{override_feature_support, remove_feature_override, Operation};
        use std::collections::HashMap;
        use url::Url;

        let feature = TableFeature::TypeWidening;
        ...
        let custom_support =
            KernelSupport::Custom(|_protocol, _properties, operation| match operation {
                Operation::Scan => Err(Error::unsupported("Custom: Scan not supported")),
                Operation::Cdf => Ok(()),
            });

        override_feature_support(feature.clone(), Some(custom_support), None);
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Custom: Scan not supported"));
